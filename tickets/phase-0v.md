...
### [x] T0V.7 First deploy · Complexity: M · USER-ASSISTED
...
Contract correction found by a real deploy: `[tool.vercel] entrypoint = "passage/main.py"` (file-path form, from T0V.4) was rejected by Vercel's Python runtime with "Use module:object format". Fixed to `entrypoint = "passage.main:app"` (dotted module path). specs/deployment.md updated to match. This is exactly the "consult docs once, fix, don't guess repeatedly" case the ticket's escalate-if anticipated — one clean fix, not multiple blind attempts.

DONE 2026-07-15. Full chain: GitHub repo (blue442/passage-simulator, personal account) ← both branches pushed, CI green. Supabase project `passage-simulator` created (us-east-2), linked, migration pushed and verified against both the direct and the pooled (Supavisor, port 6543) connection strings. Two Vercel projects created (`passage-api` root=backend/, `passage-frontend` root=frontend/), each with rootDirectory explicitly set via the API (vercel link doesn't set this for Git-triggered builds — omitting it would have silently built from repo root instead of the subdirectory). All three env vars (PASSAGE_AUTH_TOKEN, PASSAGE_DATABASE_URL pooled, CRON_SECRET) set for Production + Preview scopes — Preview "all branches" required the REST API directly, since `vercel env add <name> preview --value ... --yes` had a CLI bug that kept demanding git-branch-required confirmation no matter which flags were passed. GitHub App needed a one-time browser authorization from Steven before Vercel could read the private repo. First deploy triggered via the API using the connected GitHub source (equivalent to a real push webhook) to sidestep a separate CLI quirk (deploying from within a linked subdirectory that also has rootDirectory set doubles the path). Frontend's vercel.json proxy already pointed at the correct real domain — the placeholder guessed in T0V.4 happened to equal the actual default `passage-api.vercel.app` domain, so no update was needed there. Verified end-to-end against the live production URLs (not just localhost): /health, keepalive 401/401/200, /api/me 401/200, and the full flow through the frontend's proxy. Steven confirmed logging in and seeing the map on his phone — phase-0 exit criterion met.

Not verifiable same-day: the daily Vercel cron's first real run of /api/cron/keepalive won't show in logs for up to ~24h (Hobby cron timing is imprecise). Worth a spot-check in a day.
